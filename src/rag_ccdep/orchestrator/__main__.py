...
Pré-requisito: as engines-alvo devem estar no ar (ex.:
`python -m rag_ccdep.cli principal --port 8000`). O orquestrador NÃO importa
...
def _run_server(host: str, port: int) -> None:
    ...
    uvicorn.run("rag_ccdep.orchestrator.api:app", host=host, port=port, reload=False)
