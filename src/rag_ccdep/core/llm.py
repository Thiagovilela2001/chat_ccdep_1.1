...
--------------------------
    RAG_LLM_PROVIDER   "maritaca" (padrão) | "openai" | "openrouter" |
                       "nvidia" | "ollama"
...
OpenRouter usa `OPENROUTER_API_KEY`; NVIDIA NIM usa `NVIDIA_API_KEY`.
...
_PROVIDERS: dict[str, dict] = {
    ...
    "openrouter": {
        ...
        "interp_model": "openai/gpt-oss-120b",
        "popup_model": "openai/gpt-oss-120b",
...
def _cfg() -> dict:
    ...
    if os.getenv("RAG_LLM_MAX_TOKENS"):
        try:
            cfg["max_tokens"] = max(1, int(os.environ["RAG_LLM_MAX_TOKENS"]))
        except ValueError as exc:
            raise ValueError("RAG_LLM_MAX_TOKENS deve ser um inteiro positivo.") from exc
    if os.getenv("RAG_REASONING_EFFORT") is not None:
        cfg["reasoning_effort"] = os.environ["RAG_REASONING_EFFORT"].strip().lower()
    if os.getenv("RAG_REASONING_EXCLUDE") is not None:
        cfg["reasoning_exclude"] = os.environ["RAG_REASONING_EXCLUDE"].strip().lower() in {
            "1", "true", "yes", "on",
        }
    return cfg
...
def chat_completion_kwargs() -> dict:
    """Parâmetros extras para modelos reasoning em APIs compatíveis."""
    cfg = _cfg()
    ...
    return {
        "extra_body": {
            "reasoning": {
                "effort": effort,
                "exclude": bool(cfg.get("reasoning_exclude", True)),
            }
        }
    }
    ...
    *,
    ...
    if cfg["base_url"]:
        ...
        return OpenAI(
            ...
            additional_kwargs=chat_completion_kwargs(),
        )
