"""
Query Interpreter — versão simplificada para o Agentic RAG.

No rag_principal, o interpreter roteia para fontes específicas.
Aqui, o agente decide internamente — o interpreter apenas:
  1. Detecta is_labor_market para injetar a skill no prompt do agente.
  2. Retorna sources=["text","tables","timeseries"] (todas — agente filtra).
...
def interpret_query(question: str, llm) -> dict:
    ...
    Retorna dict compatível com evaluate.py:
        sources         : lista de fontes (todas — agente decide)
        rewritten_query : query original (agente reescreve internamente)
    ...
    log.info(
        "QueryInterpreter (agentic) | is_labor_market=%s | query: %s",
